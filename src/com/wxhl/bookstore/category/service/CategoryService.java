package com.wxhl.bookstore.category.service;

import java.sql.SQLException;
import java.util.List;

import com.wxhl.bookstore.book.dao.BookDao;
import com.wxhl.bookstore.category.dao.CategoryDao;
...
public class CategoryService {
	...
	public void deleteOneLevel(String cid) throws CategoryException {
		try {
			...
			if (cnt > 0) {
				throw new CategoryException("删除1级分类失败！该分类下还存在子分类，不能删除！");
			}
	...
	public void deleteTwoLevel(String cid) throws CategoryException {
		try {
			...
			if (cnt > 0) {
				throw new CategoryException("删除2级分类失败！该分类下还存在图书，不能删除！");
			}
			categoryDao.delete(cid);
		} catch (SQLException e) {
			throw new RuntimeException(e);
